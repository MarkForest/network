...
class RegisterForm extends Model
{
    ...
    public function getOptionsForSelect(){
        ...
        $paramsDay = ['prompt'=>'День'];

        $itemsMonth = [];
        for ($i=1;$i<=User::MONTH_AT_YEAR; $i++){
            $itemsMonth[$i]=$i;
        }
        $paramsMonth = ['prompt'=>'Мес'];

        $itemsYear = [];
        for ($i=date('Y')-User::MAX_AGE ;$i<=date('Y'); $i++){
            $itemsYear[$i]=$i;
        }
        $paramsYear = ['prompt'=>'Год'];
        ...
        $paramsCountries = ['prompt'=>'Страна'];

        return [
            'itemsDay'=>$itemsDay,
            'paramsDay'=>$paramsDay,
            'itemsMonth'=>$itemsMonth,
            'paramsMonth'=>$paramsMonth,
            'itemsYear'=>$itemsYear,
            'paramsYear'=>$paramsYear,
